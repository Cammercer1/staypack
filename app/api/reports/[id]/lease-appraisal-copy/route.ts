...
import { rebuildLeaseAppraisalFinalReport } from "@/lib/lease-appraisal/rebuildLeaseAppraisalReport";
...
export async function PATCH(
  ...
  try {
    ...
    if (!parsed || !hasLeaseAppraisalComps(parsed)) {
    ...
    if (!templateId || !isLeaseAppraisalTemplateId(templateId)) {
      return NextResponse.json(
        { error: "Choose a rental appraisal template first" },
        { status: 400 },
      );
    }
