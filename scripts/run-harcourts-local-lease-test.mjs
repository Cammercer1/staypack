/**
 * Harcourts Local → Havenly lease appraisal only (first listing on /listings/buy).
 *
 * Usage:
 *   npm run dev
 *   PRINT_BASE_URL=http://localhost:3000 npx tsx scripts/run-harcourts-local-lease-test.mjs
 */
...
const TENANT_SLUG = "havenly-harcourts-local-test";
const PARTNER_URL = "https://www.harcourtslocal.com.au/listings/buy";
...
console.log("Harcourts Local → Havenly lease appraisal test\n");
...
const listingCandidates = discovered.filter((item) => {
  try {
    const path = new URL(item.listingUrl).pathname;
    return /\/property\/[^/]+/i.test(path) || /\/listing\/[^/]+/i.test(path);
  } catch {
    return false;
  }
});
...
console.log("   Land m²:", parsed.landAreaSqm ?? "—");
...
const tenant = await upsertDeliveryTenant({
  slug: TENANT_SLUG,
  name: "havenly property (Harcourts Local test)",
...
console.log("\n3. Generating lease appraisal (REA rent discover + PDF)...");
...
if (parsed.rentalAppraisal) {
  console.log("\n   Rent band:", parsed.rentalAppraisal);
}

console.log("\nDone.");
console.log("Listing URL:", target.listingUrl);
console.log("Address:", lease.address);
