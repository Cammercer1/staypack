...
export function EditableBlurbField({
  blocks,
  onChange,
  className,
  ...
  const emitBlocks = useCallback((next: BrochureBlurbBlock[]) => {
  ...
  const editor = useEditor({
    ...
    onBlur: ({ editor: ed }) => {
      ...
      emitBlocks(tiptapDocToBlurbBlocks(ed.getJSON()));
      onFieldFocus?.(null);
    },
  });
  ...
  return (
    <div className="brochure-blurb-editor relative z-10 min-w-0 overflow-visible rounded-sm focus-within:ring-2 focus-within:ring-primary/50">
      {editor ? (
        <BubbleMenu
          editor={editor}
