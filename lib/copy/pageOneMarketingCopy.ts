...
export const pageOneMarketingCopyAiSchema = z.object({
  heading: z.string(),
  blurb_short: z.string(),
  blurb_medium_paragraphs: z.array(z.string()).length(BLURB_PARAGRAPH_COUNTS.medium),
  blurb_long_paragraphs: z.array(z.string()).length(BLURB_PARAGRAPH_COUNTS.long),
  bullets: z.array(z.string()).length(PAGE_ONE_BULLET_COUNT),
});
...
export function getPageOneMarketingCopyPromptLimits() {
  return `Hard limits (content beyond these will be truncated):
...
export function pageOneFromAiShape(raw: z.infer<typeof pageOneMarketingCopyAiSchema>): PageOneMarketingCopy {
  const paragraphs: BlurbVariantsParagraphs = enforceBlurbVariantsParagraphs({
...
export function pageOneToAiCopyJson(
  ...
  const enforced = enforcePageOneMarketingCopy(pageOne);
  const medium = blurbVariantsStoredToParagraphs(enforced.blurb_variants).medium
    .filter(Boolean)
    .join("\n\n");
  ...
  return {
    ...
    sales_pack_blurb_variants: enforced.blurb_variants,
