...
function resolveBellePhotos(document: BrochureDocumentJson) {
...
function buildBelleFeatureItems(document: BrochureDocumentJson) {
  return getPropertyHighlights(document.copy).slice(0, 4);
}

function BelleStatPill({
  value,
  label,
  text,
  mutedText,
  ...
  return (
    <div className="flex items-baseline gap-1.5">
      <span
        className="text-[1.1rem] font-belle leading-none"
...
function BelleStatStrip({
  ...
  if (
    !isLeaseAppraisalReport(report) &&
    report.property.accommodates
  ) {
    stats.push({
      value: formatNumber(report.property.accommodates),
      label: "Guests",
    });
  }
      ...
      {resolveBrochurePrice(document) ? (
        <div className="flex items-baseline gap-1.5">
          ...
          <Editable
            ...
            className="text-[1rem] font-belle"
...
function BelleHero({ document }: { document: BrochureDocumentJson }) {
  const { hero } = resolveBellePhotos(document);
  ...
  return (
    <div className="relative h-[140mm] shrink-0 overflow-hidden">
      ...
      {logoUrl ? (
        ...
        <p
          className="absolute left-7 top-6 text-sm font-belle uppercase tracking-widest text-white drop-shadow"
      ...
      <div className="absolute inset-x-0 bottom-0 px-8 pb-7">
        <p
          className="text-[0.65rem] font-semibelle uppercase tracking-[0.22em] text-white/75"
          style={{ fontFamily: headingFont }}
        >
        ...
        <h1
          className="mt-1.5 text-[1.75rem] font-belle leading-[1.1] text-white"
          style={{ fontFamily: headingFont }}
        >
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="min-w-0">
        {agent.name ? (
          <p
            className="text-[0.82rem] font-belle leading-tight text-neutral-900"
            style={{ fontFamily: headingFont }}
          >
            {agent.name}
          </p>
...
function BelleBody({
  ...
  const features = buildBelleFeatureItems(document);
    ...
    <div className="grid min-h-0 flex-1 grid-cols-[1.45fr_1fr] gap-8 overflow-hidden px-8 py-6">
      ...
      <div className="flex min-h-0 flex-col gap-4 overflow-hidden">
        <BrochureBlurbContent
          ...
          headingClassName="text-[0.72rem] font-belle uppercase tracking-wide text-neutral-900"
        />
...
export function BelleFooterBand({ document }: { document: BrochureDocumentJson }) {
  const band = resolveBrochureBrandBand(document.agency);
  ...
  return (
    ...
    >
      {logoUrl ? (
        ...
        <p
          className="text-sm font-belle uppercase tracking-[0.14em]"
          style={{ fontFamily: headingFont, color: band.text }}
        >
          {document.agency.name}
        </p>
...
/** Belle page 1 — full-bleed hero overlay, stat strip, body columns, footer. */
export function BellePageOneSpread({
  document,
  report,
}: {
  document: BrochureDocumentJson;
  ...
  return (
    <>
      <BelleHero document={document} />
      <BelleStatStrip document={document} report={report} />
      <BelleBody document={document} report={report} />
      <BelleFooterBand document={document} />
    </>
  );
}

// Keep old exports so page 2 (BelleBrochure.tsx) still compiles
export { resolveBellePhotos };
export type { BelleBodyProps };
type BelleBodyProps = { document: BrochureDocumentJson; report: FinalReportJson };
